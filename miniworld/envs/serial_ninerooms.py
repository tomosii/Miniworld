...
class SerialNinerooms(MiniWorldEnv, utils.EzPickle):
    ...
    ENV_SIZE = ROOM_SIZE * 3 + ROOM_MARGIN * 2
    ...
    def _gen_world(self):
        room0 = self.add_rect_room(
            min_x=-self.ENV_SIZE / 2,
            max_x=-self.ENV_SIZE / 2 + self.ROOM_SIZE,
            min_z=-self.ENV_SIZE / 2,
            max_z=-self.ENV_SIZE / 2 + self.ROOM_SIZE,
            floor_tex="grass",
            # wall_tex="grass",
            # no_ceiling=True,
        )
        room1 = self.add_rect_room(
            min_x=-self.ENV_SIZE / 2 + self.ROOM_SIZE + self.ROOM_MARGIN,
            max_x=-self.ENV_SIZE / 2 + self.ROOM_SIZE * 2 + self.ROOM_MARGIN,
            min_z=-self.ENV_SIZE / 2,
            max_z=-self.ENV_SIZE / 2 + self.ROOM_SIZE,
            floor_tex="cardboard",
            # wall_tex="cardboard",
